...
from metrics.losses import det_loss, jacobian_det, masked_sim_loss, ortho_loss, reg_loss, dice_jaccard, sim_loss
...
# Dataset settings
parser.add_argument('-d', '--dataset',     type=str, default='/processing/AbdomenCTCT_dataset.json', help='Specifies a data config') 
parser.add_argument('-rd', '--root_dir',   type=str, default='/processing/AbdomenCTCT/', help='Specifies the root directory where images are stored') 
...
parser.add_argument('-base', '--base_network', type=str, default='CLM')#
...
parser.set_defaults(in_channel=1 if parser.parse_args().base_network == 'CLM' else 2)
parser.set_defaults(use_affine=0 if parser.parse_args().base_network == 'FMA' else 1)
